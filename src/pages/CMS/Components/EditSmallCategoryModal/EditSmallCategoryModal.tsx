...
import { useStyles } from "./EditSmallCategoryModalStyle";
import { useState, useEffect, useRef } from "react";
import ActionButton from "components/ActionButton/ActionButton";
import ActionInput from "components/ActionInput/ActionInput";
import ConfirmModal from "components/ConfirmModal/ConfirmModal";

interface EditSmallCategoryModalProps {
  show: boolean;
  onClose: () => void;
  action: (e: any) => void;
}
export default function EditSmallCategoryModal({
  show,
  onClose,
  action,
}: EditSmallCategoryModalProps) {
    ...
    <>
      <div
        ...
        <div className={classes.modalRoot} ref={contentRef}>
          <div className={classes.title}>小さなカテゴリを編集する</div>
          <div className={classes.mainContainer}>
            <div className={classes.descContainer}>
              入力で変更される小さなカテゴリ名を正しく入力してください。
            </div>
            <ActionInput
              value={category}
      ...
      <ConfirmModal
        title='小さなカテゴリの編集'
        description={`本当に小さな${category}カテゴリを編集しますか？`}
        show={confirmModal}
